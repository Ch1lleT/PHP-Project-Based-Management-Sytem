...
@section('content')
    ...
    <script>
        ...
        function exportToCSV(plan) {
            ...
            const csvContent = rows.map(row => Array.from(row.children).map(cell => `"${cell.textContent.trim()}"`).join(',')).join('\n');
            const blob = new Blob(["\ufeff", csvContent], { type: 'text/csv;charset=utf-8;' }); // Set UTF-8 encoding
